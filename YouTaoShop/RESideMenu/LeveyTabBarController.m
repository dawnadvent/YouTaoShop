...
- (void)tabBar:(LeveyTabBar *)tabBar didSelectIndex:(NSInteger)index
{
	...
	if (self.selectedIndex == index) {
        ...
        [[nav.viewControllers objectAtIndex:0] viewDidAppear:NO];
        [self displayViewAtIndex:index];
    }
}

- (UIButton *)getTabButtonByIndex:(NSUInteger)index
{
    return [_tabBar.buttons objectAtIndex:index];
}

- (void)setTabButtonNewFlag:(NSUInteger)index
{
    UIButton *button = [_tabBar.buttons objectAtIndex:index];
    UIImageView *imageView = [[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"info_1.png"]] autorelease];
    imageView.userInteractionEnabled = NO;
    imageView.frame = CGRectMake(button.frame.size.width - 15, -2, 22, 22);
    imageView.tag = 100;
    [button addSubview:imageView];
}

- (void)removeTabButtonNewFlag:(NSUInteger)index
{
    UIButton *button = [_tabBar.buttons objectAtIndex:index];
