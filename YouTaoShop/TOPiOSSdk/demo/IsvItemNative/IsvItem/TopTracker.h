...
//api调用的结果
-(void)trackAPICall:(NSString *)method error:(NSString *)error timeConsume:(int)timeConsume data:(NSString *)data isLocal:(BOOL)isLocal forceFlush:(BOOL)forceFlush;

//应用间数据交互,如果是本地存储，isLocal设置为True
-(void)trackAPPCall:(NSString *)event appkey:(NSString *)appkey userId:(NSString*)userId isSuccess:(BOOL)isSuccess
